...
import 'package:admin/screens/agricultural/components/agricultural_detail.dart';
...
import 'agricultural_card.dart';

class AgriculturalProperties extends StatefulWidget {
  @override
  _AgriculturalPropertiesState createState() => _AgriculturalPropertiesState();
}

class _AgriculturalPropertiesState extends State<AgriculturalProperties> {
  final ValueNotifier<int> _refreshPropertiesNotifier = ValueNotifier<int>(0);
  Future<List<AgriculturalPropertyApi>>? _propertiesFuture;

  @override
  void initState() {
    super.initState();
    _propertiesFuture = fetchAllAgricultural(); // Fetch once during initialization
  }
      ...
      children: [
                ...
                showDialog(
                  ...
                  builder: (context) {
                    if(menuAppController.userPermissions.contains('Add'))
                      return  AddAgriculturalDialog(refreshPropertiesNotifier: _refreshPropertiesNotifier,);
        ...
        ValueListenableBuilder(
          ...
          builder: (context, value, child) {
            return FutureBuilder<List<AgriculturalPropertyApi>>(
              future: fetchAllAgricultural(),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  ...
                  return Text("There is no Agricultural Properties available.");
                }
      ...
      itemBuilder: (context, index) => GestureDetector(
          onTap: () {
            ...
            if (propertyId != null) {
              showDialog(
                context: context,
                builder: (context) => AgriculturalDetailDialog(propertyId: propertyId, refreshPropertiesNotifier :refreshPropertiesNotifier),
              );
            } else {
              print("Property ID is null");
            }
          },
          child: AgriculturalCard(property: properties[index], info: null, refreshPropertiesNotifier: refreshPropertiesNotifier,)
      ),
