...
import 'add_office_diaog.dart';
import 'office_card.dart';
import 'office_detail.dart';

class OfficeProperties extends StatefulWidget {
  @override
  _OfficePropertiesState createState() => _OfficePropertiesState();
}

class _OfficePropertiesState extends State<OfficeProperties> {
  final ValueNotifier<int> _refreshPropertiesNotifier = ValueNotifier<int>(0);
  Future<List<OfficePropertyApi>>? _propertiesFuture;

  @override
  void initState() {
    super.initState();
    _propertiesFuture = fetchOffices(); // Fetch once during initialization
  }
      ...
      children: [
              ...
              onPressed: () {
                showDialog(
                  ...
                  builder: (context)  {
                    ...
                    return  AddOfficeDialog(refreshPropertiesNotifier: _refreshPropertiesNotifier);
        ...
        ValueListenableBuilder(
          ...
          builder: (context, value, child) {
            return FutureBuilder<List<OfficePropertyApi>>(
              future: fetchOffices(),
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  ...
                  return Text("There is no Office Properties available.");
                }
...
class _PropertyListPageState extends State<PropertyListPage> {
  late Future<List<OfficePropertyApi>> properties;
  final ValueNotifier<int> _refreshPropertiesNotifier = ValueNotifier<int>(0);

  @override
  void initState() {
    super.initState();
    properties = fetchOffices();
  }
...
class FileInfoCardGridView extends StatelessWidget {
  final List<OfficePropertyApi> properties;
      ...
      itemBuilder: (context, index) => GestureDetector(
          onTap: () {
            ...
            if (propertyId != null) {
              showDialog(
                context: context,
                builder: (context) => OfficeDetailDialog(propertyId: propertyId, refreshPropertiesNotifier :refreshPropertiesNotifier),
              );
            } else {
              print("Property ID is null");
            }
          },
          child: OfficeCard(property: properties[index], info: null, refreshPropertiesNotifier: refreshPropertiesNotifier,)
      ),
